    ...
    async with websockets.connect('ws://127.0.0.1:9222/session', max_size=20_000_000) as ws:
        ...
        try:
            ...
            async def evaluate(expression):
                ...
                if result['type']=='exception': raise RuntimeError(result.get('exceptionDetails'))
            ...
            for width in [320,360,430,768,1280]:
                ...
                for section in ['home','products','stock','sales','finance','settings']:
                    ...
                    if width in [360,1280]: await screenshot(f'{section}-{width}')
                for modal in ['productModal','saleModal','categoryModal','colorModal','sizeModal','transactionModal']:
                    await evaluate(('openProductModal();' if modal=='productModal' else f'openModal({json.dumps(modal)});')+'new Promise(r=>setTimeout(r,40))')
                    ...
                    if width==360:
                        ...
                        if modal=='saleModal':
                            ...
                            await screenshot('sale-product-picker-360')
                            await evaluate("document.querySelector('[data-sale-select-product=\"p1\"]').click()")
                            await screenshot('sale-variant-picker-360')
                            await evaluate('closeSaleProductPicker()')
                        if modal=='productModal':
                            await evaluate('document.querySelector(".product-photo-section").scrollIntoView()')
                            await screenshot('product-photos-360')
                            await evaluate('document.querySelector(".product-variation-form").scrollIntoView()')
                            await screenshot('product-variations-360')
